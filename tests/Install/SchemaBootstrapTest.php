...
test('migraciones activas no usan ADD COLUMN IF NOT EXISTS (solo MariaDB)', function (): void {
    $files = array_merge(
        glob(ROOT_PATH . '/database/migrations/*.sql') ?: [],
        glob(ROOT_PATH . '/skeleton/database/migrations/*.sql') ?: [],
    );
    assert_true($files !== [], 'expected active migration SQL files');
    foreach ($files as $file) {
        $sql = (string) file_get_contents($file);
        assert_true(
            !str_contains($sql, 'ADD COLUMN IF NOT EXISTS'),
            basename($file) . ' uses MariaDB-only ADD COLUMN IF NOT EXISTS; '
            . 'MySQL 8.0 (platform-integration-gates) requires information_schema + PREPARE'
        );
    }
});
