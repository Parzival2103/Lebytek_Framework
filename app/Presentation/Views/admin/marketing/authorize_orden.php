...
<div class="container py-4">
  ...
  <?php elseif (! $authorizeEnabled): ?>
    <div class="alert alert-warning">La autorización está deshabilitada por configuración (MKT_MEMBERSHIP_AUTHORIZE_ENABLED=false).</div>
    <a class="btn btn-secondary" href="/admin/crud/mkt_ordenes">Volver</a>
  <?php elseif (! in_array((string) ($order['status'] ?? ''), ['pending_transfer', 'awaiting_review'], true)): ?>
    <div class="alert alert-info">Esta orden ya no está pendiente de autorización (estado: <?= ViewHelper::e((string) ($order['status'] ?? '')) ?>).</div>
    <a class="btn btn-secondary" href="/admin/crud/mkt_ordenes">Volver</a>
  <?php elseif (trim((string) ($order['api_tenant_public_id'] ?? '')) === ''): ?>
    <div class="alert alert-warning">
      Asocia el <strong>tenant demo</strong> (<code>api_tenant_public_id</code>) en el CRUD de la orden antes de autorizar.
    </div>
  ...
  <?php else: ?>
    ...
    <?php if (! empty($order['api_activation_error'])): ?>
      <div class="alert alert-danger">Último error API: <?= ViewHelper::e((string) $order['api_activation_error']) ?></div>
    <?php endif; ?>
    <p>Se activará el plan en api.lebytek.com, se revocará el token demo y se enviará el correo de membresía al cliente.</p>
    <form method="post" action="/admin/marketing/ordenes/autorizar">
      <?= ViewHelper::csrfField() ?>
      <input type="hidden" name="orden_id" value="<?= (int) $orderId ?>">
      <button type="submit" class="btn btn-primary">Confirmar autorización</button>
