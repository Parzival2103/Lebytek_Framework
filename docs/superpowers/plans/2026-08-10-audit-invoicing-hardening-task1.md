...
- [ ] **Step 1: Escribir el test que falla** — crear `tests/Invoicing/FacturapiSecretKeyValidationTest.php`:
...
- [ ] **Step 2: Ejecutar el test y comprobar el fallo** — Run: `php tests/run.php Invoicing/FacturapiSecretKeyValidation` / Expected: FAIL — `fromSecretKey` acepta `sk_live_x` con mode test (sin validación) o tests no encontrados si el archivo aún no existe.
...
- [ ] **Step 4: Verificación enfocada** — Run: `php tests/run.php Invoicing/FacturapiSecretKeyValidation Invoicing/InvoicingFactory` / Expected: PASS (6 tests nuevos + 3 existentes; happy path factory sigue verde).

- [ ] **Step 5: Regresión relevante** — Run: `php tests/run.php Invoicing` / Expected: PASS — suite Invoicing completa sin regresiones en Issue/Reconcile/Provider tests.

- [ ] **Step 6: Commit** — archivos: `src/Application/Invoicing/InvoicingFactory.php`, `src/Infrastructure/Invoicing/FacturapiInvoiceProvider.php`, `src/Infrastructure/Invoicing/Facturapi/SdkFacturapiTransport.php` (si tocado), `tests/Invoicing/FacturapiSecretKeyValidationTest.php`, `tests/Invoicing/InvoicingFactoryTest.php` — mensaje: `fix(invoicing): enforce Facturapi mode/key prefix and reject empty secrets`

## Criterios finales de aceptación

- [ ] Enabled + secret vacío → `InvoiceProviderException` sin filtrar secret.
- [ ] `mode=test` + `sk_live_*` → excepción; `mode=live` + `sk_test_*` → excepción.
- [ ] `mode=test` + `sk_test_*` válido → provider instanciable vía factory y directo.
- [ ] Ningún cambio en use cases Issue/Cancel/Reconcile.
- [ ] `php tests/run.php Invoicing` PASS.
...
| Tareas completadas / totales | 0 / 1 |
| Siguiente tarea ejecutable | Task 1 Step 1 |
| Prerrequisitos | PHP ≥8.2, `composer install`, plan padre Tasks 0 (docs) completos |
| Bloqueos | Ninguno para implementación |
