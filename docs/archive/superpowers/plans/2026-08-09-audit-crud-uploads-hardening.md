...
|------:|------|-------------------|----------|
| 1 | `2026-08-07-crud-p01-authz-multi-canal.md` | Completado | PR #95 @ `main` |
| 2 | `2026-08-07-crud-p02-states-form-options.md` | Completado | PR #100 @ `main` |
...
- [x] **Step 1: Escribir el test que falla** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — en `tests/Security/UploadValidatorTest.php`:
...
- [x] **Step 2: Ejecutar el test y comprobar el fallo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Security/UploadValidator` / Expected: FAIL — test «acepta cuando no hay lista blanca» eliminado; nuevos casos FAIL (allowlist null pasa hoy).

- [x] **Step 3: Implementar el cambio mínimo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — en `UploadValidator.php`:
...
- [x] **Step 4: Verificación enfocada** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Security/UploadValidator` / Expected: PASS (todos los casos, incluidos MIME/tamaño legacy).

- [x] **Step 5: Regresión relevante** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Archivos/FileUploadService` / Expected: PASS (caso feliz usa allowlist implícita vía config — ver Task 2 si falla por allowlist null en helper `upload_config`; ajustar helper a `['txt']` default en tests Archivos si necesario).

- [x] **Step 6: Commit** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — `git add src/Application/Services/UploadValidator.php tests/Security/UploadValidatorTest.php` · mensaje: `fix(crud): require upload allowlist and deny dangerous extensions (C6)`
...
- [x] **Step 1: Escribir el test que falla** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — añadir al final de `tests/Archivos/FileUploadServiceTest.php`:
...
- [x] **Step 2: Ejecutar el test y comprobar el fallo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Archivos/FileUploadService` / Expected: FAIL en casos jail (hoy escribe fuera o crea directorio sin validar).

- [x] **Step 3: Implementar el cambio mínimo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — en `FileUploadService.php`, extraer método privado y usarlo en L62–67:
...
- [x] **Step 4: Verificación enfocada** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Archivos/FileUploadService` / Expected: PASS.

- [x] **Step 5: Regresión relevante** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Crud/Upload/CrudUploadLedger` / Expected: PASS (rutas relativas `/uploads/...` conservadas).

- [x] **Step 6: Commit** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — `git add src/Application/Services/FileUploadService.php tests/Archivos/FileUploadServiceTest.php` · mensaje: `fix(crud): jail CRUD upload paths under public/uploads (C6)`
...
- [x] **Step 1: Escribir el test que falla** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — crear `tests/Crud/Upload/CrudConfigValidatorUploadsTest.php`:
...
- [x] **Step 2: Ejecutar el test y comprobar el fallo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Crud/Upload/CrudConfigValidatorUploads` / Expected: FAIL — método `uploadsBlockErrors` no existe / reglas ausentes.

- [x] **Step 3: Implementar el cambio mínimo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — en `CrudConfigValidator.php`:
...
- [x] **Step 4: Verificación enfocada** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Crud/Upload/CrudConfigValidatorUploads` / Expected: PASS.

- [x] **Step 5: Regresión relevante** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Crud/State/CrudConfigValidatorStates Crud/CrudConfigValidatorShape` / Expected: PASS (sin regresión validator existente).

- [x] **Step 6: Commit** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — `git add src/Application/Services/CrudConfigValidator.php tests/Crud/Upload/CrudConfigValidatorUploadsTest.php` · mensaje: `fix(crud): validate uploads block and require file allowlists (C6)`
...
- [x] **Step 1: Escribir el test que falla** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — crear `tests/Docs/CrudModuleUploadsTest.php`:
...
- [x] **Step 2: Ejecutar el test y comprobar el fallo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Docs/CrudModuleUploads` / Expected: FAIL (doc incompleta).

- [x] **Step 3: Implementar el cambio mínimo** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — en `docs/modules/crud/modulo-crud-engine.md`, ampliar la sección de uploads (≈L264) con:
...
- [x] **Step 4: Verificación enfocada** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Docs/CrudModuleUploads PlatformVersionSemver` / Expected: PASS @ `1.2.8`.

- [x] **Step 5: Regresión relevante** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Kernel/SkeletonPurity` / Expected: 13/13 PASS.

- [x] **Step 6: Commit** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — `git add docs/modules/crud/modulo-crud-engine.md tests/Docs/CrudModuleUploadsTest.php composer.json config/app.php skeleton/config/app.php` · mensaje: `docs(crud): uploads hardening runbook and bump 1.2.8 (C6)`
...
- [x] **Step 1: Escribir el test que falla** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — N/A (gate de integración). Confirmar demos cargan:
...
- [x] **Step 2: Ejecutar regresión funcional completa** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run:
...
- [x] **Step 3: Verificar avatares sin regresión (U8)** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Archivos` (incluye paths avatar si existen) y, si presente en harness, suite Avatares:
...
- [x] **Step 4: Verificación espejo demos** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run:
...
- [x] **Step 5: Regresión cross-suite** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — Run: `php tests/run.php Crud/State Crud/Action/Security` / Expected: PASS (p01/p02 no regresionados).

- [x] **Step 6: Commit** — evidencia: PR #111 (`e9f1607`), tag `v1.2.8` — solo si fixes de gate: `git add <archivos>` · mensaje: `test(crud): uploads C6 regression gate fixes`
...
- [x] **C6 config:** — PR #111 recurso con `uploads.enabled=true` y campo `file` sin `allowed_extensions` no carga (validator).
- [x] **C6 config:** — PR #111 `public_path` con `..` o fuera de patrón `uploads/...` rechazado.
- [x] **C6 runtime:** — PR #111 `UploadValidator` rechaza `null`/`[]` allowlist.
- [x] **C6 runtime:** — PR #111 denylist bloquea `php`, `svg`, etc. incluso en allowlist.
- [x] **C6 path:** — PR #111 `FileUploadService` no escribe fuera de `{PUBLIC_PATH}/uploads/`.
- [x] **U2:** — PR #111 mensaje runtime incluye extensiones permitidas cuando allowlist conocida.
- [ ] **U4/U5:** errores config vs runtime distinguibles (validator load vs POST form).
- [x] **U8:** — PR #111 avatares sin regresión.
- [x] Semver trío @ `1.2.8`; tag Git publicado post-merge y post-REL-C1.
- [x] `php tests/run.php SkeletonPurity` — verificado @ `cea890e` PASS.
...
| Campo | Valor |
|-------|-------|
| Reconciliación UTC | 2026-08-14 |
| Framework `origin/main` verificado | `cea890e34d53ad2e400237136bc691a98e030511` |
| Tareas completadas / totales | **5 / 5** |
| Evidencia implementación | PR #111 (`e9f1607`) — `UploadValidator`, `FileUploadService`, `CrudConfigValidator::uploadsBlockErrors`, docs C6, semver bump `1.2.8`; tags `v1.2.7`…`v1.2.11` publicados |
| Siguiente tarea ejecutable | — (plan cerrado) |
| Prerrequisitos | Ninguno |
| Bloqueos residuales | Portal lock bump (M6/P-LOCK) — operador manual |
| Estado | **Completo** — archivado 2026-08-14 |
