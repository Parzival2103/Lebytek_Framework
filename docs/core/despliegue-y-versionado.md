...
```
  ...
  4. Onboarding vertical dom_* T4              Según el módulo (ver Relación con otros documentos)
...
### Inventario maestro de superficies
...
Leyenda de columnas:
- **Tier:** T1–T4 según Modelo conceptual: 4 tiers.
...
| `config/vertical.php` | T3 (excepción) | Repo | No | No | No | Sí | Toggle de módulos activos + labels de menú |
...
> ¹ **`.env`:** cambiarlo no reinicia el proceso en PHP por-request, pero si hay OPcache/persistencia conviene recargar para garantizar lectura fresca; ver Checklist pre/post deploy por entorno.
...
### Playbooks por tipo de despliegue
...
- **Riesgo principal:** checksum modificado tras aplicar (drift) → ver Versionado y actualización; no re-ejecutar a ciegas.
...
- **Riesgo principal:** olvidar una dependencia (la resuelve el `DependencyResolver`); inconsistencia de clave entre `vertical.php` y el manifiesto (ver Deuda conocida y decisiones abiertas).
...
- **Responde:** *"¿Cómo agrego un dominio de negocio nuevo?"* → remite a [`vertical-onboarding.md`](vertical-onboarding.md) y [`../modules/uso-de-modulo-dominio.md`](../modules/uso-de-modulo-dominio.md).
...
### Dos números independientes con semver
...
La página `/admin/sistema/estado` (y `status.php`) comparan la **versión declarada** en el manifiesto contra la **versión instalada** en `cfg_modulos`. Si difieren, marcan "actualización disponible". Esto ya está implementado; esta guía solo fija *cuándo* el desarrollador debe mover cada número.

### Procedimiento ante checksum modificado
...
2. Si el cambio ya está reflejado manualmente en la BD y el `.sql` se editó por error, la alerta indica **drift**: documentarlo. (Re-sellar el checksum es una *decisión abierta* — ver Deuda conocida y decisiones abiertas — porque hoy no hay comando para ello.)
3. La alerta de `status.php` es señal de **drift**, no un error que se "arregla" re-corriendo el archivo.

### Baseline para deploys legacy
...
|---|---|---|---|
| Document root en `public/` | N/A (se pasa `-t public`) | Sí | **Crítico** (ver [`despliegue_hosting.md`](despliegue_hosting.md)) |
...
Reglas resumidas (la fuente autoritativa es [`seguridad_secretos_deploy.md`](seguridad_secretos_deploy.md); aquí solo el extracto operativo):
...
Hallazgos detectados al explorar el repo. Se documentan como deuda; resolverlos requiere su propio spec/plan porque tocan código/config.

| # | Hallazgo | Impacto | Seguimiento propuesto |
|---|---|---|---|
| D1 | Claves con guion bajo en `config/vertical.php` (`pdf_kit`, `reportes`) vs guion en manifiesto (`pdf-kit`). | Confusión al cruzar toggle ↔ manifiesto; potencial fallo silencioso si algo casa por clave exacta. | Decidir una convención única de clave y alinear. |
| D2 | Spec archivado 2026-06-08 desactualizado: no menciona `bootstrap_sql`, `pdf-kit` ni `reportes`. | Lectura engañosa si se toma como guía operativa vigente. | Marcado como histórico; su modelo de datos sigue válido, su narrativa operativa la reemplaza esta guía. |
| D3 | No existe comando para **re-sellar** un checksum tras un cambio legítimo. | El drift solo se puede documentar, no limpiar; alertas persistentes. | Evaluar un `--reseal`/equivalente en un spec futuro; hoy decisión abierta (ver Versionado y actualización). |
| D4 | `vertical.modules` mezcla slugs de menú raíz con claves de módulo. | Modelo mental ambiguo sobre qué representa cada clave del toggle. | Aclarar la semántica de `vertical.modules` (¿toggle de módulo o de menú?). |
...
Esta guía es la **puerta de entrada única de despliegue**. No absorbe contenido: delega los temas profundos en sus dueños y los referencia.

| Documento | Relación | Qué aporta / queda |
|---|---|---|
| [`instalacion-y-versionado.md`](instalacion-y-versionado.md) | **Complementa** | Sigue siendo dueño de la mecánica del motor (manifiestos, superficies, baseline). Esta guía añade el modelo de tiers y la guía operativa. |
| [`despliegue_hosting.md`](despliegue_hosting.md) | **Complementa** | Sigue siendo dueño del detalle Apache / document root. El playbook de hosting compartido enlaza aquí. |
| [`seguridad_secretos_deploy.md`](seguridad_secretos_deploy.md) | **Complementa** | Sigue siendo dueño de secretos y rotación. El resumen de seguridad es solo extracto. |
| [`vertical-onboarding.md`](vertical-onboarding.md) | **Complementa** | Sigue siendo dueño del onboarding T4. El Playbook 4 delega aquí y en `uso-de-modulo-dominio.md`. |
| [`../modules/uso-de-modulo-dominio.md`](../modules/uso-de-modulo-dominio.md) | **Complementa** | Checklist oficial de módulo de dominio (T4). |
| [`../archive/superpowers/specs/2026-06-08-instalacion-estandarizacion-versionado-design.md`](../archive/superpowers/specs/2026-06-08-instalacion-estandarizacion-versionado-design.md) | **Obsoleta (parcial)** | Su **modelo de datos `cfg_*` y el motor siguen vigentes**. Su **narrativa operativa** queda superada por esta guía. No usar como guía de despliegue. |

**Qué reemplaza esta guía:** el rol de "guía de despliegue" que estaba disperso entre los docs anteriores y el spec archivado, ahora centralizado aquí como índice operativo único. El diseño completo y los criterios de aceptación viven en el spec [`../superpowers/specs/2026-06-14-despliegue-versionado-madurez-design.md`](../superpowers/specs/2026-06-14-despliegue-versionado-madurez-design.md).
