...
WhatsApp → readiness → ejecución → cierre). Es la versión **genérica** de
`docs/automation/` de `Lebytek_Framework`: mismos invariantes y ciclo de vida,
sin acoplar el prompt a un solo producto.

Úsalo para instalar la cadena en otro repo (p. ej. `WhatsApiLebytek`) sin
copiar literales de Framework (`src/`, Payments, skeleton, Marketing ownership).
...
## Cómo instalar en otro repo

1. Copia **todo** este directorio a `docs/automation/` del repo destino
   (o fusiona con el existente; no dejes dos cadenas incompatibles).
2. Copia un perfil:
   ```bash
   cp docs/automation/profiles/WhatsApiLebytek.md docs/automation/REPO-PROFILE.md
   # o: cp docs/automation/REPO-PROFILE.example.md docs/automation/REPO-PROFILE.md
   #    y rellena los campos
   ```
3. Asegura carpetas de artefactos (crear vacías o con `.gitkeep` si no existen):
   - `docs/audits/`
...
## Relación con `docs/automation/` de este repo

| Ubicación | Audiencia |
|-----------|-----------|
| `docs/automation/` | Cadena **de producción** de `Lebytek_Framework` (prompts acoplados al paquete) |
| `docs/automation-kit/` | Plantilla portable para **otros** repos |

No sustituyas los prompts de Framework por este kit en las automations ya
