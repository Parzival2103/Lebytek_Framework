...
CREATE TABLE IF NOT EXISTS `dom_mkt_leads` (
  ...
  `api_instance_public_id` CHAR(26) NULL,
  `external_ref`  VARCHAR(255)    DEFAULT NULL,
  `api_provisioned_at` DATETIME   DEFAULT NULL,
  `demo_started_at` DATETIME      DEFAULT NULL,
  ...
  `first_authorized_at` DATETIME  DEFAULT NULL,
  `first_message_sent_at` DATETIME DEFAULT NULL,
  `api_provision_error` TEXT       DEFAULT NULL,
  `api_lifecycle_status` VARCHAR(32) NOT NULL DEFAULT 'none',
  ...
  KEY `idx_mkt_leads_demo_expires` (`demo_expires_at`),
  KEY `idx_mkt_leads_last_activity` (`last_activity_at`),
  KEY `idx_mkt_leads_plan_slug` (`plan_slug`),
